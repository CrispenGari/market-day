import { AUDIOS, COLORS, DELIVERY_OPTIONS, FONTS } from "@/src/constants";
...
const ItemForm = ({ category }: { category: "produce" | "stock" }) => {
  const { settings } = useSettingsStore();
  const router = useRouter();
  const { camera, gallery, requestCameraPermission } = useMediaPermissions();
  const { me } = useMeStore();
  const publishItemMutation = useMutation(api.api.items.publish);
  ...
  const publish = async () => {
    ...
    if (!!!me) {
      return setState((s) => ({
        ...
        loading: false,
        error:
          "You are not authenticated so you can not publish products on Mzansi Mart.",
      }));
    }
    ...
    const id = await publishItemMutation({
      image: file.id,
      userId: me._id,
      type: category,
      title: state.title,
    ...
    if (!!id) {
      ...
      Alert.alert(
        "Mzansi Mart",
        "Product was listed successfully in the market.",
        [
          {
            text: "OK",
            style: "cancel",
